...
import { Audio, AudioTranscribeParams, AudioTranscribeResponse } from './resources/audio';
import {
  BatchCancelResponse,
  BatchCreateParams,
  BatchCreateResponse,
...
export interface ClientOptions {
  /**
   * Defaults to process.env['LLM_MULTIPLE_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['LLM_MULTIPLE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['LLM_MULTIPLE_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Llm Multiple API.
 */
export class LlmMultiple {
  apiKey: string | null;
   ...
   * API Client for interfacing with the Llm Multiple API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['LLM_MULTIPLE_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['LLM_MULTIPLE_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('LLM_MULTIPLE_BASE_URL'),
    apiKey = readEnv('LLM_MULTIPLE_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? LlmMultiple.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('LLM_MULTIPLE_LOG'), "process.env['LLM_MULTIPLE_LOG']", this) ??
      defaultLogLevel;
  ...
  protected stringifyQuery(query: Record<string, unknown>): string {
    return Object.entries(query)
      ...
      .map(([key, value]) => {
        ...
        throw new Errors.LlmMultipleError(
  ...
  static LlmMultiple = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static LlmMultipleError = Errors.LlmMultipleError;
